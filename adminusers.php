...
<!-- intro_video_bg start-->
    ...
    <style type="text/css">
        ...
        .report{position: relative;left: 160px;top: 60px;} 
                ...
                ?>
                    ...
                    <div id="newid"><b>ADD NEW MANAGERS</b>
                        <form method="post">
                            ...
                            <table >
                            <tbody>
                            <tr><td>User name</td><td>:&nbsp;<input type="text" name="name" required value="<?php echo "$uname" ?>"></td></tr>
                            <tr><td>Email</td><td>:&nbsp;<input type="email" name="email" required value="<?php echo "$email" ?>"></td></tr>
                            <tr><td>Contact</td><td>:&nbsp;<input type="number" name="contact" required value="<?php echo "$contact" ?>"></td></tr>
                            <tr><td>Address</td><td>:&nbsp;<textarea name="address" required ><?php echo "$contact" ?></textarea></td></tr>
                   ...
                   <table border style="width: 150%;text-align: center;">
                       <thea d class="heading"><th>USER ID</th><th>USER NAME</th><th>EMAIL ID</th><th>CONTACT NO</th><th>ADDRESS</th><th>PASSWORD</th><th>STATUS</th><th><span class='ti-settings'></span></th><th>Info</th></thead>
                               <?php
                                ...
                                {
                                    echo "<tr><td>{$row['user_id']}</td><td>{$row['username']}</td><td>{$row['email']}</td><td>{$row['contact']}</td><td>{$row['address']}</td><td>{$row['password']}</td><td>{$row['ac_status']}</td><td>
                                        <a href='adminuser_change_delete.php?did={$row['user_id']}'><span class='ti-reload' title='Change status'></span></a></td><td><a href='updateuser.php?id={$row['user_id']}'>Update</a></td></tr>";
                                }
